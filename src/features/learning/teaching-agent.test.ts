...
describe("teaching agent", () => {
  ...
  it("prepares one simple lesson with a text-free concept image and no relationship image", async () => {
    ...
    expect(lesson.conceptVisual.imagePrompt).toContain("知识卡片、表格、笔记、思维导图、流程图、关系图、信息图")
    expect(lesson.conceptVisual.imagePrompt).not.toContain("来源依据")
    expect(lesson.conceptVisual.imagePrompt).not.toContain(context.sourceExcerpt)
    expect(lesson.conceptVisual.cacheFingerprint).toContain("pure-visual-v4")
